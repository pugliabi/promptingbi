...
I want a loop, and a loop needs two harnesses. The **context harness** is where the project's knowledge lives and gets refined. The **execution harness** is where the work happens. I've written about the pieces one at a time in the [Stop Re-Prompting](/2026/07/20/stop-re-prompting-second-brain-agent-instructions/) series: the [hub](/2026/07/29/anatomy-of-a-project-hub/), the [assistants](/2026/08/03/meet-my-assistants/), and the [MCP execution session](/2026/08/05/inside-an-mcp-execution-session/). This post puts them in one picture. It's the talk I gave at the [Chicago Fabric / Power BI User Group](https://www.meetup.com/chicagolandpowerbi/events/315787998/) this week, written down, because the pieces only make sense once you see what each harness is FOR.
...
<figure>
  <img src="/images/2026/09/context-harness-execution-harness-banner.png" alt="Two harnesses side by side: a context harness on the left with a project hub and related pages, an execution harness on the right with two workbenches, and MCP connectors reaching into Fabric and Power BI, joined by a loop" loading="lazy" />
  <figcaption>The two harnesses: context on the left, execution on the right, and the recap that closes the loop.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-execution-harness-architecture.png" alt="The context harness on the left holding a hub and its related pages, an arrow carrying an instructions page into the execution harness on the right, MCP connector nodes reaching from the execution harness into a lakehouse and a semantic model, and a dotted return arrow carrying a recap back to the hub" loading="lazy" />
  <figcaption>Context goes in as an instructions page, work comes out through the MCP servers, and the recap goes back to the hub.</figcaption>
</figure>
...
The basics are in the series. [The project is the hub](/2026/07/29/anatomy-of-a-project-hub/) and everything else is a relation, so an agent can follow the links between pages to find an answer. Every milestone page uses the same skeleton, so the agent always knows where each section is. And [the hub writes the instructions](/2026/08/05/inside-an-mcp-execution-session/): role, #1 priority, do-not-touch list, read-first table, report-back.
...
<figure>
  <img src="/images/2026/09/context-harness-master-sequence-current-phase.png" alt="The top of the Northside master sequence page in Notion: a blue Current Phase callout listing what&#x27;s done, the Sep 24 workspace gap, three ordered focus steps, and a do-not-start line, followed by the status legend for active, complete, paused, deferred, and planned pages" loading="lazy" />
  <figcaption>The top of the Northside master sequence: the dated Current Phase callout and the status legend every session reads first.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-master-sequence-phases.png" alt="The Instruction Page Sequence section of the Northside master sequence: Phase 0 Discovery and Data Platform and Phase 1 Semantic Model with completed and planned items, each linking to its instructions page with close-out notes, and Phase 2 Report Design with the Desktop Bridge page marked active" loading="lazy" />
  <figcaption>The Instruction Page Sequence. Each item links to its instructions page and says when it closed and where the leftover work went.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-master-sequence-waves.png" alt="The Wave A and Wave B lists from the Northside master sequence: three active Wave A ticketing report pages with their specs, two planned Wave A items for an audit log page and a calculation reference document, and Wave B Operations and Event P&amp;L marked paused until Wave A is approved, with four planned items naming the gold tables and page specs each one needs" loading="lazy" />
  <figcaption>Waves inside the active phase. PLANNED items wait for their pages, and Wave B waits for Wave A's sign-off.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-agent-flow.png" alt="A flow diagram from the Desktop Bridge instructions page: the Fabric Notebooks pipeline agent builds bronze, silver, and gold into the NBC Data Dictionary and Gold Definition, the Semantic Model Builder reads source tables and writes the model back to Notion, and the Desktop Bridge and Report Design agent reads columns and measures to design visuals and screenshot report pages" loading="lazy" />
  <figcaption>The Agent Flow diagram on the Desktop Bridge page: whose output this agent reads, and who reads its output.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-desktop-bridge-instructions.png" alt="The top of the Desktop Bridge and Report Design instructions page in Notion: a green status callout marking it active as of Sep 24 with the model state and scope, a Purpose section defining the agent as a report design agent, and a list of its three tools, the Power BI Desktop Bridge, the Report Authoring Skill, and the Notion MCP server" loading="lazy" />
  <figcaption>The active instructions page: a dated status callout, a one-paragraph role, and exactly three tools.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-report-workflow-loop.png" alt="The Core Workflow Loop diagram from the Desktop Bridge instructions: read Notion requirements and schema, edit PBIR files, reload Desktop, screenshot pages, review screenshots, then iterate back to editing or finish when approved" loading="lazy" />
  <figcaption>The execute step for report work: edit the PBIR files, reload Desktop, screenshot, review, and iterate.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-desktop-bridge-warnings.png" alt="The Critical Warnings section of the Desktop Bridge instructions: seven numbered warnings learned on earlier engagements, including that MCP model changes never update Desktop&#x27;s Power Query layer, that multi-value card visuals render empty, and to never save in Desktop between an agent write-back and the verify run" loading="lazy" />
  <figcaption>Lessons from earlier engagements, written into the page so no future session has to relearn them.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-execution-harness-loop.png" alt="A circular loop of four nodes around a central project hub: an instructions page, a magnifying-glass survey, a build step reaching into a lakehouse and a semantic model, and a recap document flowing back into the hub" loading="lazy" />
  <figcaption>Every session runs the same four steps around the hub: instruct, recon, execute, write back.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-validation-benchmarks.png" alt="The Validation Benchmarks table at the bottom of the Northside master sequence, listing expected values and their sources: 71 lakehouse tables and 32.3 million rows, 75 foreign-key and 23 grain checks with zero orphans, 7,271,674 FactTicketSales rows, $236,175,062 gross ticket revenue, the no-filter anchor game, STH account counts, the season margin range, and the 9-table semantic model v1" loading="lazy" />
  <figcaption>The expected values, each with its source, written down before the build.</figcaption>
</figure>
...
<figure>
  <img src="/images/2026/09/context-harness-desktop-bridge-session-workflow.png" alt="The Session Workflow section of the Desktop Bridge instructions: nine numbered steps from reading Notion pages and checking Desktop state through design, reload, screenshot, validate, and iterate, ending with step 9, write back, update status on this page and the master sequence page in Notion" loading="lazy" />
  <figcaption>Every instructions page ends its session workflow with the write-back.</figcaption>
</figure>

And the master sequence spells out exactly what "write back" means:

<figure>
  <img src="/images/2026/09/context-harness-write-back-protocol.png" alt="The Context Write-Back Protocol on the Northside master sequence: six numbered steps covering the dated write-back on the instruction page, flipping the status on the master sequence, updating the Data Resource Index, ticking milestone deliverables, flagging discrepancies without overwriting silently, and stopping at scope edges" loading="lazy" />
  <figcaption>What "write back" means on Northside, step by step.</figcaption>
</figure>
