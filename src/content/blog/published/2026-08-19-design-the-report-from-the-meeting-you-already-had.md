...
The first time I pointed an agent at a finished semantic model and said "here are the measures, go design a report," I got something back. It was a report. It had visuals on it. It was also completely sporadic. Run the same request twice and I got two unrelated pages, neither of which I'd show a client.
...
Nothing changed about the tooling between those two runs. What changed was the **context**. Agents can author reports now. Whether you can hand over what you already know is the whole ballgame.
...
The [Power BI authoring plugin](https://github.com/microsoft/skills-for-fabric/tree/main/plugins/powerbi-authoring) is split three ways:
...
Microsoft put planning first and gave it its own skill. The mechanics were never the hard part.

A measure list tells an agent what is POSSIBLE, not what is wanted. Your model exposes forty measures. A good executive page shows three. Nothing in the TMDL tells an agent which three, or why the CFO cares about margin variance this quarter and not last quarter's volume.
...
Three inputs turned my results around:
...
What I do instead is lock that context into one brief before the agent touches a file. Not a second round of requirements gathering. A written spec of what the meeting already told me:
...
This is the same move `powerbi-report-planning` makes when it refuses to author anything until a spec is approved. That skill will happily interview you to build one, and I would rather not sit through an interview about a conversation I already had. So I bring the meeting instead.

The brief is the meeting, written down so both an agent and a human can execute against it. Context lives in an artifact you keep next to the project, in source control. [Not in a chat history](/2026/07/20/stop-re-prompting-second-brain-agent-instructions/).
...
The skills already know this. They shell out to a CLI. `@microsoft/powerbi-report-authoring-cli` installs as `powerbi-report-author`, and two habits from it are worth stealing even if you never install a single skill: look the spec up, and validate constantly.
...
These are the ways a hand-written visual fails, and not one of them announces itself:
...
Run validation after every batch, not at the end. A broken field reference surfaces on the step that caused it instead of four steps later when you can't tell which change did it. Invalid PBIR gets rejected on reload anyway, so you may as well hear it from the validator.
...
That changes where the agent spends its effort. It stops thinking about file format and starts thinking about what the page looks like, where the KPIs go, what the general layout should be. That's the work I want it doing, because that's the work the brief told it how to do.

Push formatting into the theme, not into visuals. Static formatting belongs in the theme, and conditional formatting belongs in extension measures returning theme sentiment tokens. Then variance columns carry the color and nothing else does, because formatting everything means formatting nothing. The first time you re-theme a report, per-visual color overrides beat the theme every time, and the "one line" change turns into a sweep through every file on disk.
...
I had this wrong too. The [Power BI Desktop Bridge](https://learn.microsoft.com/en-us/power-bi/developer/agentic/power-bi-desktop-bridge-overview) is a local IPC server running inside the Desktop process, reachable over a named pipe scoped to that process ID, speaking JSON-RPC. Local only. There is no remote surface, which is the first thing your security people will ask.
...
Everything else is your agent editing PBIP files on disk. Desktop is the renderer and the camera. Before the Bridge, every iteration cost you a Desktop restart, so nobody iterated. An agent that can render and then look at what it rendered is a different animal from one that writes files and hopes.
...
The skills install their CLIs globally by default, which means a client project's tooling lands on your whole machine. GitHub Copilot agent hooks let you intercept that and rewrite a global install into a project-local dependency, so the tools stay scoped to the repo they belong to. Same instinct as a virtual environment, and worth doing before your second project rather than your fifth.
...
I have had the documented guidance tell me to use one visual type while the screenshot showed it rendering completely empty in the build I was on. The docs weren't lying and neither was the screenshot. That disagreement is why the verify step exists, and why "the agent said it built it" and "the agent built it" are still two different claims.

Brief in, commands out, screenshot back, compare, refine.
...
- A measure list is inventory. The requirement is the stakeholder meeting you already had: transcript, reference screenshots, and the metrics that came up.
- Lock that into a design brief with two to five decision questions before a single visual is built. Anything that serves no decision question is decoration.
...
- The Desktop Bridge reloads and screenshots. Wrap validate, reload, and screenshot in a script so verification has a file-system answer.
...
Agentic report design rewards the people who were already good at requirements. The tooling caught up. The discipline is still on us. If you're building this loop out for your own team, keep the conversation going with us on the Explicit Measures podcast, and subscribe at PromptingBI for more.
